...
{
    const Teuchos::RCP<const Epetra_MpiComm> Comm = 
        CTrilinos::tableRepos().getConst<Epetra_MpiComm, 
        CT_Epetra_MpiComm_ID_t>(CommID);
    return CTrilinos::tableRepos().store<Epetra_MpiComm, 
        CT_Epetra_MpiComm_ID_t>(new Epetra_MpiComm(*Comm));
}
  ...
  CT_Epetra_MpiComm_ID_t selfID )
{
    return CTrilinos::tableRepos().store<Epetra_Comm, CT_Epetra_Comm_ID_t>(
        CTrilinos::tableRepos().getConst<Epetra_MpiComm, 
        CT_Epetra_MpiComm_ID_t>(selfID)->Clone());
}

void Epetra_MpiComm_Barrier ( CT_Epetra_MpiComm_ID_t selfID )
{
    CTrilinos::tableRepos().getConst<Epetra_MpiComm, CT_Epetra_MpiComm_ID_t>(
        selfID)->Barrier();
}
  ...
  int Root )
{
    return CTrilinos::tableRepos().getConst<Epetra_MpiComm, 
        CT_Epetra_MpiComm_ID_t>(selfID)->Broadcast(MyVals, Count, Root);
}
...
{
    return CTrilinos::tableRepos().getConst<Epetra_MpiComm, 
        CT_Epetra_MpiComm_ID_t>(selfID)->GatherAll(MyVals, AllVals, Count);
}
...
{
    return CTrilinos::tableRepos().getConst<Epetra_MpiComm, 
        CT_Epetra_MpiComm_ID_t>(selfID)->SumAll(PartialSums, GlobalSums, 
        Count);
}
...
{
    return CTrilinos::tableRepos().getConst<Epetra_MpiComm, 
        CT_Epetra_MpiComm_ID_t>(selfID)->MaxAll(PartialMaxs, GlobalMaxs, 
        Count);
}
...
{
    return CTrilinos::tableRepos().getConst<Epetra_MpiComm, 
        CT_Epetra_MpiComm_ID_t>(selfID)->MinAll(PartialMins, GlobalMins, 
        Count);
}
...
{
    return CTrilinos::tableRepos().getConst<Epetra_MpiComm, 
        CT_Epetra_MpiComm_ID_t>(selfID)->ScanSum(MyVals, ScanSums, Count);
}
  ...
  CT_Epetra_MpiComm_ID_t selfID )
{
    return CTrilinos::tableRepos().store<Epetra_Distributor, 
        ...
        CTrilinos::tableRepos().getConst<Epetra_MpiComm, 
        CT_Epetra_MpiComm_ID_t>(selfID)->CreateDistributor());
}
